<script>import { addHours, addMinutes, addSeconds, startOfDay } from 'date-fns';
...
import { endDate, endHour, endMinute, endSecond, getTimezone, relativeTimeDuration, relativeTimeUnit, startDate, startHour, startMinute, startSecond, TIME_UNIT_OPTIONS, timeFormat, timeFormatType, } from '../../stores/time-format';
import { getSelectedTimezone } from '../../utilities/format-date';
import { toDate } from '../../utilities/to-duration';
...
const onApply = () => {
    if ($timeFormatType === 'relative' && !useBetweenDateTimeQuery) {
        ...
        const formattedStartTime = zonedTimeToUtc(startDateWithTime, timezone).toISOString();
        const formattedEndTime = zonedTimeToUtc(endDateWithTime, timezone).toISOString();
