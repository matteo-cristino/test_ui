...
let progressPercent;
$: {
    const { completeOperationCount, failureOperationCount, totalOperationCount, } = operation;
    completePercent = Math.round((completeOperationCount / totalOperationCount) * 100);
    failurePercent = Math.round((failureOperationCount / totalOperationCount) * 100);
    progressPercent = Math.round(((completeOperationCount + failureOperationCount) / totalOperationCount) *
        100);
}
</script>
