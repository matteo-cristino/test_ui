...
import { getFailedOrPendingEvents, getFailedOrPendingGroups, } from './get-failed-or-pending';
...
describe('getFailedOrPendingEvents', () => {
    it('should return all items if not filtering for failed event', () => {
        const filteredEvents = getFailedOrPendingEvents([completedEvent, failedEvent], false);
        expect(filteredEvents.length).toEqual(2);
    });
    it('should return no items if no failed/timedout/pending events', () => {
        const filteredEvents = getFailedOrPendingEvents([completedEvent, canceledEvent, terminatedEvent], true);
        expect(filteredEvents.length).toEqual(0);
    });
    it('should return filtered items for failed event', () => {
        const filteredEvents = getFailedOrPendingEvents([completedEvent, failedEvent], true);
        expect(filteredEvents.length).toEqual(1);
    });
    it('should return filtered items for timed out event', () => {
        const filteredEvents = getFailedOrPendingEvents([completedEvent, timedOutEvent], true);
        expect(filteredEvents.length).toEqual(1);
    });
    it('should return filtered items for pending events', () => {
        const filteredEvents = getFailedOrPendingEvents([completedEvent, pendingActivity, pendingActivity], true);
        expect(filteredEvents.length).toEqual(2);
    });
    it('should return filtered items for failed local event', () => {
        const filteredEvents = getFailedOrPendingEvents([completedEvent, failedLocalEvent], true);
        expect(filteredEvents.length).toEqual(1);
    });
});
describe('getFailedOrPendingGroups', () => {
    it('should return all items if not filtering for failed group', () => {
        const filteredGroups = getFailedOrPendingGroups([completedEventGroup, completedEventGroup], false);
        expect(filteredGroups.length).toEqual(2);
    });
    it('should return no items if no failed/canceled/pending  group', () => {
        const filteredGroups = getFailedOrPendingGroups([completedEventGroup, completedEventGroup], true);
        expect(filteredGroups.length).toEqual(0);
    });
    it('should return filtered items for failed group', () => {
        const filteredGroups = getFailedOrPendingGroups([completedEventGroup, failedEventGroup], true);
        expect(filteredGroups.length).toEqual(1);
    });
    it('should return filtered items for pending group', () => {
        const filteredGroups = getFailedOrPendingGroups([completedEventGroup, pendingEventGroup], true);
        expect(filteredGroups.length).toEqual(1);
    });
});
