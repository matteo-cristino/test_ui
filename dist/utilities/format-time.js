import { formatDuration as durationToString, getMilliseconds as getSecondAsMilliseconds, intervalToDuration, parseJSON, } from 'date-fns';
import { has } from './has';
...
export function formatDuration(duration, delimiter = ', ') {
    if (duration === null || !duration)
        return '';
    if (typeof duration === 'string')
        return fromSeconds(duration, { delimiter });
    return durationToString(duration, { delimiter });
}
export function formatDurationAbbreviated(duration, delimiter) {
    return formatDistanceToSingleLetters(formatDuration(duration, delimiter));
}
function formatDistanceToSingleLetters(distance) {
    if (!distance)
        return '';
    distance = distance
...
export function getMillisecondDuration({ start, end, onlyUnderSecond = true, }) {
    if (!start || !end)
        return null;
    try {
        if (isTimestamp(start)) {
...
export function formatDistance({ start, end, includeMilliseconds = false, includeMillisecondsForUnderSecond = false, }) {
...
export function formatDistanceAbbreviated({ start, end, includeMilliseconds = false, includeMillisecondsForUnderSecond = false, }) {
    const duration = getDuration({ start, end });
    const distance = formatDuration(duration, ' ');
...
export function fromSecondsToMinutesAndSeconds(seconds) {
    if (!seconds)
        return '';
...
export function fromSecondsToDaysOrHours(seconds) {
    if (!seconds)
        return '';
