...
import { getPendingActivity, isAssociatedPendingActivity, } from './pending-activities';
import pendingActivityWorkflow from '$fixtures/workflow.pending-activities.json';
const getScheduledEvent = (activityId) => toEvent({
    eventId: '12',
...
describe('getPendingActivity', () => {
    it('should return pending activity if the event has activityId of same pending activityId', () => {
        ...
        expect(getPendingActivity(getScheduledEvent('1'), pendingActivities)).toBe(pendingActivities[0]);
    });
    it('should return undefined if the event has different activityId of pending activityId', () => {
        const pendingActivities = pendingActivityWorkflow.pendingActivities;
        expect(getPendingActivity(getScheduledEvent('23'), pendingActivities)).toBe(undefined);
    });
});
describe('isAssociatedPendingActivity', () => {
    it('should return true if the event has matching activityId of pending activity', () => {
        const pendingActivities = pendingActivityWorkflow.pendingActivities;
        expect(isAssociatedPendingActivity(getScheduledEvent('1'), pendingActivities[0])).toBe(true);
    });
    it('should return undefined if the event has different activityId of pending activityId', () => {
        const pendingActivities = pendingActivityWorkflow.pendingActivities;
        expect(isAssociatedPendingActivity(getScheduledEvent('1'), pendingActivities[1])).toBe(false);
    });
    it('should return undefined if the event has different activityId of pending activityId', () => {
        const pendingActivities = pendingActivityWorkflow.pendingActivities;
        expect(isAssociatedPendingActivity(getScheduledEvent('16'), pendingActivities[0])).toBe(false);
    });
});
