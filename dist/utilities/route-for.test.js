...
import { hasParameters, isEventHistoryParameters, isEventParameters, isNamespaceParameter, isWorkflowParameters, routeForArchivalWorkfows, routeForAuthentication, routeForCallStack, routeForEventHistory, routeForEventHistoryImport, routeForLoginPage, routeForNamespace, routeForNamespaces, routeForPendingActivities, routeForSchedule, routeForScheduleCreate, routeForSchedules, routeForTaskQueue, routeForWorkers, routeForWorkflow, routeForWorkflowQuery, routeForWorkflows, routeForWorkflowsWithQuery, } from './route-for';
describe('routeFor', () => {
    ...
    it('should route to "workflows with query"', () => {
        ...
        expect(path).toBe('/namespaces/default/workflows?query=ExecutionStatus%3D%22Running%22');
    });
    ...
    it('should route to pending activities', () => {
        ...
        expect(path).toBe('/namespaces/default/workflows/abc/def/pending-activities');
    });
...
describe('routeFor import ', () => {
    ...
    it('should return the correct route for routeForSchedules', () => {
        expect(routeForSchedules({ namespace: 'default' })).toBe('/namespaces/default/schedules');
    });
    it('should return the correct route for routeForSchedule', () => {
        expect(routeForSchedule({ namespace: 'default', scheduleId: '123' })).toBe('/namespaces/default/schedules/123');
    });
    it('should return the correct route for routeForScheduleCreate', () => {
        expect(routeForScheduleCreate({ namespace: 'default' })).toBe('/namespaces/default/schedules/create');
    });
});
describe('routeFor SSO authentication ', () => {
    ...
    it('should fallback to the originUrl if returnUrl is not provided', () => {
        ...
        expect(sso).toBe(`${settings.baseUrl}auth/sso?returnUrl=${encodeURIComponent(originUrl)}`);
    });
    it('should use the returnUrl if provided', () => {
        ...
        const sso = routeForAuthentication({ settings, searchParams, originUrl });
        expect(sso).toBe(`${settings.baseUrl}auth/sso?returnUrl=${encodeURIComponent(returnUrl)}`);
    });
    ...
    it('Should add return URL search param', () => {
        ...
        const ssoUrl = new URL(sso);
        expect(ssoUrl.searchParams.get('returnUrl')).toBe('https://localhost/some/path');
    });
    ...
    it('test of the signin flow', () => {
        ...
        const searchParams = new URLSearchParams('invitation=Wwv6g2cKkfjyqoLxnCPUCfiKcjHKpK%5B%E2%80%A6%5Dn9ipxcao0jKYH0I3&organization_name=temporal-cloud');
        const sso = routeForAuthentication({ settings, searchParams });
        const ssoUrl = new URL(sso);
        expect(ssoUrl.searchParams.get('one')).toBeNull();
        expect(sso).toEqual('https://localhost/auth/sso?organization_name=temporal-cloud&invitation=Wwv6g2cKkfjyqoLxnCPUCfiKcjHKpK%5B%E2%80%A6%5Dn9ipxcao0jKYH0I3');
    });
    describe('routeForLoginPage', () => {
        ...
        it('should return a URL with the correct returnUrl', () => {
            ...
            expect(routeForLoginPage()).toBe('https://temporal.io/login?returnUrl=https%3A%2F%2Ftemporal.io%2Fcurrent-page');
        });
        it('should return a URL with the correct returnUrl', () => {
    ...
    describe('hasParameters', () => {
        it('should return true if it has all of the parameters', () => {
            expect(hasParameters('namespace', 'workflow')({ namespace: 'default', workflow: 'workflow-id' })).toBe(true);
        });
        it('should return false a parameter is missing', () => {
            expect(hasParameters('namespace', 'workflow')({ namespace: 'default' })).toBe(false);
        });
        it('should return true if all of the required parametersisWorkflowParameters$x are provided', () => {
            expect(isWorkflowParameters({
                namespace: 'default',
                workflow: 'workflow',
                run: 'run',
            })).toBe(true);
        ...
        it('should return true if all of the required parameters for isEventHistoryParameters are provided', () => {
            expect(isEventHistoryParameters({
                namespace: 'default',
                workflow: 'workflow',
                run: 'run',
                view: 'feed',
