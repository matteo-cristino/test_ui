...
import { convertPayloadToJsonWithCodec, decodeAllPotentialPayloadsWithCodec, decodePayload, decodePayloadAttributes, } from './decode-payload';
import { dataConvertedWorkflowStartedEvent, getTestPayloadEvent, getTestPayloadEventWithNullEncodedAttributes, noRemoteDataConverterWorkflowStartedEvent, workflowStartedEvent, workflowStartedHistoryEvent, } from './decode-payload-test-fixtures';
...
import { codecEndpoint, lastDataEncoderStatus, resetLastDataEncoderSuccess, } from '../stores/data-encoder-config';
...
describe('decodePayload with returnDataOnly = false', () => {
    ...
    it('Should not decode a payload with encoding binary/null', () => {
        ...
        expect(decodePayload(BinaryNullEncodedNoData, false)).toEqual(fullDecodedPayload);
    });
    it('Should decode a payload with encoding json/plain', () => {
        const fullDecodedPayload = {
            metadata: {
    ...
    it('Should decode a payload with encoding json/protobuf with messageType', () => {
        const fullDecodedPayload = {
            metadata: {
                encoding: 'json/protobuf',
                messageType: 'temporal.api.workflow.v1.WorkflowExecutionStartedEventAttributes',
            },
            data: Base64Decoded,
        };
        expect(decodePayload(ProtobufEncodedWithMessageType, false)).toEqual(fullDecodedPayload);
    });
    it('Should decode a json payload with encoding json/plain', () => {
        const fullDecodedPayload = {
            metadata: {
    ...
    it('Should decode a json payload with constructor keyword with encoding json/plain', () => {
        ...
        expect(decodePayload(JsonObjectEncodedWithConstructor, false)).toEqual(fullDecodedPayload);
    });
});
...
describe('decode all potential payloads', () => {
    it('Should decode a payload with codec endpoint with encoding json/plain`', async () => {
        const event = await decodeAllPotentialPayloadsWithCodec(getTestPayloadEvent(), 'default', {}, '');
    ...
    it('Should not decode a null payload with codec endpoint with encoding json/plain`', async () => {
        const event = await decodeAllPotentialPayloadsWithCodec(getTestPayloadEventWithNullEncodedAttributes(), 'default', {}, '');
        expect(event.input).toEqual({ payloads: ['test@test.com'] });
...
describe('convertPayloadToJsonWithCodec', () => {
    ...
    it('Should not include credentials with the request for cookie based authentication of data converters by default', async () => {
        ...
        expect(mockFetch).toBeCalledWith(expect.any(String), expect.not.objectContaining({ credentials: 'same-origin' }));
    });
...
describe('getEventAttributes', () => {
    ...
    it('Should convert a payload through data-converter and set the success status when the endpoint is set locally and the endpoint connects', async () => {
        ...
        const decodedPayload = await getEventAttributes({
            historyEvent: parseWithBigInt(stringifyWithBigInt(workflowStartedHistoryEvent)),
            namespace: 'default',
            settings: {
                codec: {
                    endpoint: '',
