...
import { formatDate, formatUTCOffset, getLocalTime, getSelectedTimezone, getUTCString, isValidDate, } from './format-date';
describe('formatDate', () => {
    ...
    it('should format other timezones', () => {
        expect(formatDate(date, 'Greenwich Mean Time')).toEqual('2022-04-13 GMT 16:29:35.63');
        expect(formatDate(date, 'Central Standard Time')).toEqual('2022-04-13 CDT 11:29:35.63');
        expect(formatDate(date, 'Pacific Daylight Time')).toEqual('2022-04-13 PDT 09:29:35.63');
    });
    ...
    it('should not format other timezones as relative', () => {
        expect(formatDate(date, 'UTC', { relative: true })).toEqual('2022-04-13 UTC 16:29:35.63');
    });
    it('should format relative local time with a custom label', () => {
        expect(formatDate(date, 'local', { relative: true, relativeLabel: 'custom' })).toContain('custom');
    });
    it('should shorten format for local and other timezones', () => {
        expect(formatDate(date, 'local', { abbrFormat: true })).toEqual('2022-04-13 16:29:35 PM');
        expect(formatDate(date, 'utc', { abbrFormat: true })).toEqual('2022-04-13 16:29:35 PM');
    });
    it('should shorten format without seconds if there are none for local and other timezones', () => {
        ...
        expect(formatDate(dateWithoutSeconds, 'local', { abbrFormat: true })).toEqual('2022-04-13 16:29 PM');
        expect(formatDate(dateWithoutSeconds, 'utc', { abbrFormat: true })).toEqual('2022-04-13 16:29 PM');
    });
});
...
describe('getSelectedTimezone', () => {
    it('should get the abbreviation for the timezone', () => {
        expect(getSelectedTimezone('Mountain Standard Time')).toBe('Mountain Standard Time (MST)');
    });
    it('should get the local timezone', () => {
...
describe('getUTCString', () => {
    ...
    it('should return a string with hours, minutes, and seconds set', () => {
        expect(getUTCString({ hour: 1, minute: 1, second: 1 })).toBe('2020-01-01T01:01:01.000Z');
        expect(getUTCString({ hour: 1, minute: 61, second: 1 })).toBe('2020-01-01T02:01:01.000Z');
        expect(getUTCString({ hour: 1, minute: 1, second: 61 })).toBe('2020-01-01T01:02:01.000Z');
