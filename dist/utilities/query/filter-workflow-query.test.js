...
describe('toListWorkflowQueryFromFilters', () => {
    ...
    it('should convert multiple ExecutionStatus filters', () => {
        ...
        expect(query).toBe('(`ExecutionStatus`="Running" OR `ExecutionStatus`="Canceled" OR `ExecutionStatus`="Failed")');
    });
    ...
    it('should convert three different filters', () => {
        ...
        const query = toListWorkflowQueryFromFilters(combineFilters(filters));
        expect(query).toBe('`ExecutionStatus`="Running" AND `WorkflowId`="abcd" AND `WorkflowType`="cronWorkflow"');
    });
    it('should convert datetime filter', () => {
        ...
        const query = toListWorkflowQueryFromFilters(filters, supportsAdvancedVisibility);
    ...
    it('should convert two filters, one as datetime filter', () => {
        ...
        const supportsAdvancedVisibility = isVersionNewer('1.20', '1.19');
        const query = toListWorkflowQueryFromFilters(combineFilters(filters), supportsAdvancedVisibility);
        expect(query).toBe('`WorkflowType`="cronWorkflow" AND StartTime > "2019-12-30T00:00:00.000Z"');
    });
    it('should convert a KeywordList filter', () => {
        ...
        const query = toListWorkflowQueryFromFilters(combineFilters(filters));
        expect(query).toBe('`CustomKeywordListField`in("Hello", "World") AND `CustomKeywordListField` is null AND `CustomKeywordListField`="Hello"');
    });
});
