...
import { combineDropdownFilters, combineFilters, toListWorkflowFilters, } from './to-list-workflow-filters';
const executionStatusQuery = '`ExecutionStatus`="Completed"';
const multipleExecutionStatusQuery = '(`ExecutionStatus`="Canceled" OR `ExecutionStatus`="Failed" OR `ExecutionStatus`="Completed")';
...
const betweenTimeQuery = '`StartTime` BETWEEN "2023-07-28T00:00:00-00:00" AND "2023-07-28T06:00:00-00:00"';
const workflowQuery2 = '`WorkflowType`="World" AND `StartTime` > "2022-04-18T17:45:18-06:00"';
const workflowQuery3 = '`WorkflowType`="World" AND `StartTime` > "2022-04-18T17:45:18-06:00" AND `ExecutionStatus`="Canceled"';
const workflowQuery4 = '(`ExecutionStatus`="Canceled" OR `ExecutionStatus`="Failed" OR `ExecutionStatus`="Completed") AND WorkflowType="World" AND StartTime > "2022-04-18T17:45:18-06:00"';
const customAttributesWithSpacesQuery = '`Custom Bool Field`=true AND `Custom Keyword Field`="Hello world"';
const workflowQueryWithSpaces = '`WorkflowId`="One and Two" AND `Custom Keyword Field`="Hello = world"';
...
describe('toListWorkflowFilters', () => {
    ...
    it('should parse a query with custom attributes that have spaces', () => {
        const result = toListWorkflowFilters(customAttributesWithSpacesQuery, attributes);
        const expectedFilters = [
            {
    ...
    it('should parse a query with multiple executionStatuses', () => {
        const result = toListWorkflowFilters(multipleExecutionStatusQuery, attributes);
        const expectedFilters = [
            {
                attribute: 'ExecutionStatus',
                type: 'Keyword',
    ...
    it('should parse a query with a startTime', () => {
        vi.useFakeTimers().setSystemTime(parseISO('2022-04-20T17:45:18-06:00').getTime());
    ...
    it('should parse a query with a KeywordList type and other types', () => {
        const result = toListWorkflowFilters(keywordListQuery + ' AND ' + workflowQuery4 + ' AND ' + keywordListQuery, attributes);
        const expectedFilters = [
            {
                attribute: 'CustomKeywordListField',
                type: 'KeywordList',
        ...
        const expectedFilters = [
            {
                ...
                operator: '',
                parenthesis: '',
                value: 'BETWEEN "2023-07-28T00:00:00-00:00" AND "2023-07-28T06:00:00-00:00"',
                customDate: true,
            },
        ];
        expect(result).toEqual(expectedFilters);
    ...
    it('should parse a query with a workflowId and BETWEEN two times', () => {
        vi.useFakeTimers().setSystemTime(parseISO('2022-04-20T17:45:18-06:00').getTime());
        const result = toListWorkflowFilters(`${workflowIdQuery} AND ${betweenTimeQuery}`, attributes);
        const expectedFilters = [
            {
                attribute: 'WorkflowId',
                type: 'Keyword',
    ...
    it('should not throw if given an invalid time in a BETWEEN query', () => {
        expect(() => {
            toListWorkflowFilters('StartTime BETWEEN "bogus" AND "2023-07-28T06:00:00-00:00"', attributes);
        }).not.toThrow();
        expect(() => {
            toListWorkflowFilters('StartTime BETWEEN "2023-07-28T00:00:00-00:00" AND "bogus"', attributes);
        }).not.toThrow();
    });
...
describe('combineFilters', () => {
    ...
    it('should parse a query with IS NULL and IS NOT NULL', () => {
        const result = toListWorkflowFilters(`${isEmptyQuery} AND ${isNotEmptyQuery}`, attributes);
        const expectedFilters = [
            {
                attribute: 'WorkflowType',
                type: 'Keyword',
    ...
    it('should parse a query with "is" and "is not" as a value', () => {
        const result = toListWorkflowFilters('`WorkflowId`="is" AND `WorkflowType`="is not"', attributes);
        const expectedFilters = [
            {
                attribute: 'WorkflowId',
                type: 'Keyword',
