...
vi.mock('../utilities/request-from-api', () => ({
    requestFromAPI: vi.fn().mockImplementation((route) => {
        if (route.includes('/describe')) {
            return new Promise((resolve) => resolve({ state: 'Completed', completedOperationCount: '10' }));
        }
...
describe('Batch Service', () => {
    ...
    describe('when temporal server version is <= 1.19', () => {
        beforeEach(() => {
            vi.spyOn(temporalVersion, 'subscribe').mockImplementation((fn) => () => fn('1.19'));
        });
        test('batchTerminateWorkflows calls the create batch operation endpoint with the correct options', async () => {
            ...
            expect(requestFromAPI).toHaveBeenCalledWith('http://localhost:8233/api/v1/namespaces/default/batch-operations/xxx', {
                notifyOnError: false,
                options: {
                    method: 'POST',
    ...
    describe('when temporal server version is > 1.19', () => {
        beforeEach(() => {
            vi.spyOn(temporalVersion, 'subscribe').mockImplementation((fn) => () => fn('1.20'));
        });
        test('batchTerminateWorkflows calls the create batch operation endpoint with the correct options', async () => {
            await batchTerminateWorkflows({
                namespace: 'default',
