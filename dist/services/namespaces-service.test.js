...
const createSuccessfulRequest = () => vi.fn(() => Promise.resolve({
    ok: true,
    status: 200,
...
const createUnsuccessfulRequest = () => vi.fn(() => Promise.resolve({
    ok: false,
    status: 500,
    statusText: 'error',
    json: () => Promise.resolve({}),
...
describe('fetchNamespaces', () => {
    ...
    it('should call fetch with the correct route', async () => {
        ...
        expect(request).toHaveBeenCalledWith('http://localhost:8233/api/v1/namespaces?', {
            credentials: 'include',
            headers: {
