import { isActivityTaskCanceledEvent, isActivityTaskCancelRequestedEvent, isActivityTaskCompletedEvent, isActivityTaskFailedEvent, isActivityTaskStartedEvent, isActivityTaskTimedOutEvent, isChildWorkflowExecutionCanceledEvent, isChildWorkflowExecutionCompletedEvent, isChildWorkflowExecutionFailedEvent, isChildWorkflowExecutionStartedEvent, isChildWorkflowExecutionTerminatedEvent, isChildWorkflowExecutionTimedOutEvent, isExternalWorkflowExecutionSignaledEvent, isNexusOperationCanceledEvent, isNexusOperationCompletedEvent, isNexusOperationFailedEvent, isNexusOperationStartedEvent, isPureWorkflowTaskFailedEvent, isTimerCanceledEvent, isTimerFiredEvent, isWorkflowExecutionUpdateCompletedEvent, isWorkflowTaskCompletedEvent, isWorkflowTaskStartedEvent, isWorkflowTaskTimedOutEvent, } from '../../utilities/is-event-type';
export const getGroupId = (event) => {
    ...
    if (isActivityTaskCancelRequestedEvent(event)) {
        return String(event.activityTaskCancelRequestedEventAttributes.scheduledEventId);
    }
    ...
    if (isChildWorkflowExecutionStartedEvent(event)) {
        return String(event.childWorkflowExecutionStartedEventAttributes.initiatedEventId);
    }
    if (isChildWorkflowExecutionTerminatedEvent(event)) {
        return String(event.childWorkflowExecutionTerminatedEventAttributes.initiatedEventId);
    }
    if (isChildWorkflowExecutionCompletedEvent(event)) {
        return String(event.childWorkflowExecutionCompletedEventAttributes.initiatedEventId);
    }
    if (isChildWorkflowExecutionCanceledEvent(event)) {
        return String(event.childWorkflowExecutionCanceledEventAttributes.initiatedEventId);
    }
    if (isChildWorkflowExecutionFailedEvent(event)) {
        return String(event.childWorkflowExecutionFailedEventAttributes.initiatedEventId);
    }
    if (isChildWorkflowExecutionTimedOutEvent(event)) {
        return String(event.childWorkflowExecutionTimedOutEventAttributes.initiatedEventId);
    }
    ...
    if (isWorkflowExecutionUpdateCompletedEvent(event)) {
        return String(event.workflowExecutionUpdateCompletedEventAttributes.acceptedEventId);
    }
    if (isExternalWorkflowExecutionSignaledEvent(event)) {
        return String(event.externalWorkflowExecutionSignaledEventAttributes.initiatedEventId);
    }
    ...
    if (isNexusOperationCompletedEvent(event)) {
        return String(event.nexusOperationCompletedEventAttributes.scheduledEventId);
    }
