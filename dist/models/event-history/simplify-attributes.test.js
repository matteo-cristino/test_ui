...
import { canBeSimplified, getValueForFirstKey, simplifyAttributes, } from './simplify-attributes';
const attributes = {
    workflowType: {
...
describe('simplifyAttributes', () => {
    it('should take single key attributes and reduce them down to their values', () => {
        const { initiator } = simplifyAttributes(attributes);
        expect(initiator).toBe('Unspecified');
    });
    it('should take single key attributes and reduce them down to their values', () => {
        const { parentWorkflowExecution } = simplifyAttributes(attributes);
    ...
    it('should format a duration', () => {
        const { workflowRunTimeout } = simplifyAttributes(attributes);
        expect(workflowRunTimeout).toBe('20 seconds');
    });
    it('should format a duration', () => {
        const { workflowType } = simplifyAttributes(attributes);
...
describe('canBeSimplified', () => {
    ...
    it('should return false if given an object with more than key', () => {
        expect(canBeSimplified({ key: 'value', anotherKey: 'another value' })).toBe(false);
    });
