...
describe('createPaginationStore with default pageSizeOptions', () => {
    ...
    it('should set default values', () => {
        ...
        const { key, loading, updating, index, hasNextIndexData, indexStart, indexEnd, activeIndex, indexData, pageSize, previousPageSize, hasNext, hasPrevious, } = get(store);
    ...
    it('should update store correctly on first nextPageWithItems() with empty items', () => {
        ...
        const { index, hasNextIndexData, indexStart, indexEnd, hasPrevious, indexData, } = get(store);
        expect(index).toBe(0);
        expect(hasNextIndexData).toBe(false);
        expect(indexStart).toBe(0);
        expect(indexEnd).toBe(0);
    ...
    it('should update store correctly on multiple nextPageWithItems() and previousPage', () => {
        ...
        store.previousPage();
        const { index, hasNextIndexData, indexStart, indexEnd, hasPrevious, hasNext, indexData, } = get(store);
        expect(index).toBe(1);
...
describe('createPaginationStore with custom pageSizeOptions', () => {
    ...
    it('should set default values', () => {
        ...
        const { key, loading, updating, index, hasNextIndexData, indexStart, indexEnd, indexData, pageSize, previousPageSize, activeIndex, hasNext, hasPrevious, } = get(store);
        expect(key).toBe('per-page');
        expect(loading).toBe(true);
        expect(updating).toBe(false);
        expect(index).toBe(0);
